      ...
      <div className="absolute top-0 left-0 w-full h-48 pointer-events-none"
        style={{
          background: `
            ...
            radial-gradient(ellipse at 80% 30%, rgba(0, 240, 255, 0.04) 0%, transparent 50%)
          `,
        }}
      />
      ...
      <div className="flex flex-col flex-1 z-10">
        ...
        <nav className="flex-1 px-3 space-y-0.5">
          {TABS.map((tab) => (
            ...
            >
              {activeTab === tab.id && (
                <div className="absolute left-0 top-1 bottom-1 w-[3px] rounded-r bg-[#6366f1]" />
              )}
              <span className={`flex items-center justify-center w-6 h-6 rounded-md transition-all duration-200 ${
                activeTab === tab.id
                  ...
                  : "text-white/40 bg-white/[0.03] group-hover:text-white/60 group-hover:bg-white/[0.06]"
              }`}>
                {tab.icon}
              </span>
              <span>{tab.label}</span>
      ...
      <div className="px-4 py-3 relative z-10 border-t border-white/[0.04] space-y-3">
          ...
          <div className="relative">
            ...
            {(isRecording || isProcessing) && (
              <div className={`absolute inset-[-3px] rounded-full border ${
                isRecording ? "border-red-500/30" : "border-[#6366f1]/30"
              } animate-ring-pulse`} />
            )}
          </div>
        ...
        <div className="flex items-center justify-between">
          ...
          >
            ...
            <span className="text-[11px] font-medium text-white/50 group-hover:text-white/70 transition-colors leading-tight">API Key</span>
          </button>
